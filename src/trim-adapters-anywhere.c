/*
 * Trim primers whereever they are in the sequence. We hash the primers and then look through the sequence to see if we have that hash
 *
...
int main(int argc, char *argv[]) {
	...
	if (primerfile == NULL || fqfile == NULL || outfile == NULL) {
		print_usage();
		exit(EXIT_FAILURE);
	}
	...
	trim_seq_at_primers(fqfile, primers, kmer, outfile, verbose);
